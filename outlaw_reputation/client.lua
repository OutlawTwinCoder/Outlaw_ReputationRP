...
RegisterCommand('myrep', function()
    ESX.TriggerServerCallback('outlaw_reputation:cb:getOwnDashboard', function(payload)
        if payload then
            toggleNui(true, payload)
        else
            TriggerEvent('esx:showNotification', 'Impossible de charger vos données')
        end
    end)
end)
